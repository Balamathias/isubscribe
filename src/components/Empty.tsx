...
const Empty = ({color, content, icon, className, title}: EmptyProps) => {
  return (
    <div className={clsx('flex flex-col gap-4', className)}>
      <Card className="bg-white p-4 h flex flex-col items-center justify-center gap-y-2 md:gap-y-2.5 rounded-xl border-none shadow-none drop-shadow-none">
            ...
            {
                content ? <p className="text-gray-600 text-sm text-center md:text-base">{content}</p> :
