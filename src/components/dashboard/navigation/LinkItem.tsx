...
const LinkItem = ({ link }: { link: {href: string, icon: LucideIcon, tooltip: string } }) => {
  ...
  return (
    <Link className='flex gap-3 flex-col space-y-1 py-1.5 cursor-pointer transition-all' href={link.href}>
        <div className={clsx('flex items-center gap-1.5 justify-between dark:hover:bg-gray-900 hover:bg-violet-100 hover:text-violet-950 p-2.5 rounded-md', {'bg-primary text-violet-50': isActive})}>
