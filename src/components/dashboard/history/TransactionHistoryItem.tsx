...
const TransactionHistoryItem = ({ transaction, type }: { 
    ...
    if (type === 'wallet_fund') return <Wallet {...transaction} amount={eventData?.amountPaid?.toString()!} />
    if (type === 'data_topup') return <Data {...transaction} amount={dataMetadata?.amount!} />
    if (type === 'airtime_topup') return <Data {...transaction} amount={dataMetadata?.amount!} />
    return (
    <></>
    )
}

const Wallet = ({id,...transaction}: Tables<'history'> & { amount: string }) => {
    return (
...
const Data = ({id,...transaction}: Tables<'history'> & { amount: string }) => {
    return (
        <Link href={'/dashboard/history/' + id} className='flex flex-row gap-x-2 items-center shadow-none border-none rounded-none last:border-none border-muted p-2 py-3 first:border-none hover:bg-violet-100 hover:transition-all hover:duration-300 w-full cursor-pointer justify-between border-b border'>
        <div className='flex items-center flex-row space-x-3'>
