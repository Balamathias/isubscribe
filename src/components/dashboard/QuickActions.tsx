            ...
            quickActionsLinks.map((link,idx) => (
                <div key={idx} className=' '>
                    <Link href={link?.href} className={` flex flex-col gap-1 items-center justify-center`}>
                        <link.icon size={32} className={ "p-2 rounded-xl hover:opacity-80 text-violet-600 dark:text-violet-500 bg-violet-100 dark:bg-gray-900/80 hover:p-1 hover:bg-violet-200 hover:rounded-lg hover:duration-200 hover:transition-all"} />
