...
const SubDataProvider = ({ children, action='data', wallet, profile }: SubDataProviderProps) => {
    ...
    const handleSubData = async (payload: SubDataProps & { method?: PaymentMethod }) => {
        ...
        try {
            ...
            const msg = error?.message?.includes('fetch failed') || error?.message?.includes('destructure') ? 
            `Network error, please confirm that you were debited before performing another transaction` : error?.message
            toast.error(msg)
            console.error(error)
            setErrorMessage(msg || 'Data subscription failed. Please try again.')
            setPurchasing(false)
            setOpenConfirmPurchaseModal(false)
            setPurchaseFailed(true)
        } finally {
